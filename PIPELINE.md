...
Implemented in `backend/app/pipeline/reprocessing.py` (the pure matching decision — supersede/new/flag-ambiguous/flag-conflict, including the case two candidates in the same run both match one existing letter) and `backend/app/pipeline/reprocessing_service.py` (wired to the actual database writes). Both are exercised against a live PostgreSQL instance in `backend/tests/`, not just unit-tested in isolation — see § Verification status.
...
| Job lease recovery | **Implemented and tested against a live crash simulation.** `backend/app/pipeline/jobs.py` — `claim_next_job()` (`FOR UPDATE SKIP LOCKED`), `reap_expired_leases()` (the reaper query), `complete_job()`. 10 integration tests in `backend/tests/test_jobs_integration.py` against a real PostgreSQL 18 instance, including two genuinely concurrent connections proving SKIP LOCKED behavior a single connection cannot exercise (a job locked by an uncommitted transaction is invisible to a second worker, not blocking, not double-claimed) and the full crash story end to end: claim → backdate the lease to simulate a dead worker → reap → reclaim by a *different* worker id, with `attempts` correctly bumped on both. Confirmed non-vacuous by breaking the reaper's `WHERE` clause and watching exactly the two dependent tests fail. |
| Pipeline stages (S0–S8) | Design only, except S6 reprocessing-matching below. No other pipeline code written yet. |
| Reprocessing / letter-versioning matching logic (S6) | **Implemented, tested, and wired to real database writes.** `backend/app/pipeline/reprocessing.py` (pure match logic, 11 unit tests) plus `backend/app/pipeline/reprocessing_service.py`, which loads current letters, runs the match, and executes the outcome: the three-statement supersede, a plain insert for a genuinely new letter, or an insert-and-flag (fresh serial, `needs_review`, a `review_events` row) for an ambiguous or conflicting match. 4 integration tests against live Postgres confirm the actual bug this thread started from is fixed: after a supersede, the `register` view returns **exactly one row**, and the full version history remains queryable. |
