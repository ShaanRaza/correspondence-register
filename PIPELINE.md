...
| Reprocessing / letter-versioning matching logic (S6) | **Implemented and tested.** `backend/app/pipeline/reprocessing.py` — pure function, no DB dependency, taking the current letters for one (package, document) and a new run's candidates, returning a match decision per candidate (supersede/new/flag-ambiguous/flag-conflict). 11 tests in `backend/tests/test_reprocessing.py`, run from a clean venv with `pip install -e backend[dev] && pytest backend`, all passing. One test's assertion was confirmed non-vacuous by deliberately breaking the conflict-detection branch and watching that specific test fail, then restoring it. Not yet wired to a real database call — the caller still owns loading `existing_letters` and executing the three-statement supersede sequence documented above. |
