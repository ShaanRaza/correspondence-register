...
`db/schema.sql` revision 3 was applied clean to the same PostgreSQL 18.6 + pgvector 0.8.6 instance used for round 1, then exercised with a second smoke-test script covering: `extraction_runs` status/finished_at/is_current CHECK constraints (both directions), `letters`' page-range CHECK, all four new composite FKs (thread-by-package, party-by-package, field-by-run, field-by-page), the full letter-versioning supersede sequence with the register view confirmed to show one current row and the complete history still queryable, `refresh_letter_review_status()`, and `letter_embeddings`' multi-model uniqueness (including its partial HNSW index, which pgvector 0.8.6 accepted without complaint).

**Update:** the S6 reprocessing-match heuristic named above as not-yet-implemented now is. `backend/app/pipeline/reprocessing.py` (pure decision logic) and `backend/app/pipeline/reprocessing_service.py` (wired to real writes) are covered by 15 tests total — 11 pure-logic, 4 against live PostgreSQL — plus 10 more for job-lease crash recovery (`backend/app/pipeline/jobs.py`). Two real bugs surfaced only by running the integration tests, both fixed: `psycopg` returns UUID columns as `uuid.UUID` objects, and comparing one to a plain `str` id is silently `False` even when the values match — every such comparison now normalizes with `str()`; and the job-queue tests' own cleanup fixture deadlocked on first run, because pytest tears fixtures down in the reverse of their setup order and the cleanup step's `DELETE` ran before the connection holding the row's lock had rolled back — fixed by wiping the table at the *start* of each test instead of relying on teardown ordering. Added `review_action.flagged` to the schema (a pipeline-authored review event, distinct from the three human-authored ones) once wiring the ambiguous-match case up revealed there was no action value that actually fit a machine-generated note.

Not yet exercised: `llm_requests` (created, not yet exercised with real multi-call data), and a full pipeline run end to end (S0–S8) against this schema.
